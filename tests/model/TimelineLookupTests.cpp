...
TEST_CASE("the tempo at a tick is the last change at or before it", "[timeline][lookup]")
{
    ...
    CHECK(seq.getTimeline().getTempoAt(1919) == 120.0);
    CHECK(seq.getTimeline().getTempoAt(1920) == 90.0);
    CHECK(seq.getTimeline().getTempoAt(3839) == 90.0);
    CHECK(seq.getTimeline().getTempoAt(3840) == 150.0);
    CHECK(seq.getTimeline().getTempoAt(100000) == 150.0);
    const auto change = seq.getTimeline().getTempoChangeAt(2000);
...
TEST_CASE("before the first tempo change the default tempo applies", "[timeline][lookup]")
{
    ...
    CHECK(seq.getTimeline().getTempoAt(479) == 120.0);
    const auto before = seq.getTimeline().getTempoChangeAt(479);
    CHECK(before.tick == 0);
    CHECK(before.bpm == 120.0);

    seq.setTempoChanges({});
    CHECK(seq.getTimeline().getTempoAt(0) == 120.0);
    const auto empty = seq.getTimeline().getTempoChangeAt(0);
    CHECK(empty.tick == 0);
...
TEST_CASE("the time signature at a tick is the last change at or before it", "[timeline][lookup]")
{
    ...
    CHECK(seq.getTimeline().getTimeSignatureAt(7679) == TimeSignatureChange{0, 4, 4});
    CHECK(seq.getTimeline().getTimeSignatureAt(7680) == TimeSignatureChange{7680, 3, 4});
    CHECK(seq.getTimeline().getTimeSignatureAt(100000) == TimeSignatureChange{7680, 3, 4});
}
...
TEST_CASE("before the first time signature change 4/4 applies", "[timeline][lookup]")
{
    ...
    CHECK(seq.getTimeline().getTimeSignatureAt(1919) == TimeSignatureChange{0, 4, 4});
    seq.setTimeSignatureChanges({});
    CHECK(seq.getTimeline().getTimeSignatureAt(0) == TimeSignatureChange{0, 4, 4});
}
