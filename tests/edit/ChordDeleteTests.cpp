...
{
    CHECK(ChordTrackEdits::afterDelete(joinedRun(), {1}) ==
...
TEST_CASE("deletion normalizes redundant no-chords elsewhere", "[chord][delete]")
{
    ...
    CHECK(ChordTrackEdits::afterDelete(before, {3}) == chords({{0, "C"}, {1920, "N.C."}}));
}

TEST_CASE("invalid or no-chord indices are a no-op", "[chord][delete]")
{
    const auto before = chords({{0, "C"}, {1920, "N.C."}, {3840, "F"}, {5760, "N.C."}});
    CHECK(ChordTrackEdits::afterDelete(before, {}) == before);
    CHECK(ChordTrackEdits::afterDelete(before, {-1, 99}) == before);
    CHECK(ChordTrackEdits::afterDelete(before, {1, 3}) == before);
}
